		...
		for (int z = 0; z < zdim; z++) {
			for (int y = 0; y < ydim; y++) {
				for (int x = 0; x < xdim; x++) {
					if (z == 0 || z == zdim_1 || (x - xdim / 2.0) * (x - xdim / 2.0)
						+ (y - ydim / 2.0) * (y - ydim / 2.0) <= z*z / Zco)
			...
			for (int y = 0; y < ydim; y++) {
				for (int z = 0; z < zdim; z++) {
					if (material.at(x, y, z) == Material::fluid) {
						if (sqrt((x - xdim / 2.0) * (x - xdim / 2.0)
							+ (y - ydim / 2.0) * (y - ydim / 2.0)
							+ (z - 45) * (z - 45)) <= 30) {

                            rho.at(x, y, z) = rho_liquid;

                            /*
							if ((x - xdim / 2.0) * (x - xdim / 2.0)
								+ (y - ydim / 2.0) * (y - ydim / 2.0) > z*z / Zco + 200) {
								rho.at(x, y, z) = (rho_liquid + rho_gas) / 2.0
									- (rho_liquid - rho_gas) / 2.0
									* tanh(2.0*(sqrt(double((x - xdim / 2.0) * (x - xdim / 2.0)
								...
								if (material.at(xp, yp, zp) == Material::fluid)
                                    ...
                                    */
									...
									if (zp == 0 || zp == zdim_1) {
										...
										double local_x = (x - xdim / 2.0);
										double local_y = (y - ydim / 2.0);
										double q = 0;
                                        ...
                                        if ((t1 < 0 && t1 >= -1 - 1e-5) && (t2 < 0 && t2 >= -1 - 1e-5)) {
                                            if (abs(t1) > abs(t2)) {
                                                q = -t2;
                                            }
                                            else {
										...
										if (zppp <= 0 || zppp >= zdim_1) {
                                            if (q < 0.5) {
                                                ...
                                                F.at(x, y, z, k) = (2 * q - 1) * (FC.at(x, y, z, r[k]) - FC.at(xpp, ypp, zpp, r[k])) + FC.at(x, y, z, r[k]);
                                            }
                                            /*
											F.at(x, y, z, k) = FC.at(x, y, z, r[k]);
                                            //throw 0;
										...
										if (q < 0.5) {
											...
											*/
                                            /*
											F.at(x, y, z, k) = q *(1 + 2 * q) * FC.at(x, y, z, r[k]) + (1 + 2 * q) * (1 - 2 * q) * FC.at(xpp, ypp, zpp, r[k])
												- q * (1 - 2 * q) * FC.at(xppp, yppp, zppp, r[k]);
                                            */
                                            F.at(x, y, z, k) = 2 * q * FC.at(x, y, z, r[k]) + (1 - 2 * q) * FC.at(xpp, ypp, zpp, r[k]);
										}
...
#pragma omp parallel for
		for (int x = 0; x < xdim; x++)
			for (int y = 0; y < ydim; y++)
				for (int z = 0; z < zdim; z++) {
					if (material.at(x, y, z) == Material::fluid) {
						if (abs(WF_Max_before) < abs(WF.at(x, y, z)))
							WF_Max_before = WF.at(x, y, z);
						for (int k = 0; k < Q; k++) {
		...
		cout << "WF_before" << WF_Max_before << endl;
			...
			for (int y = 0; y < ydim; y++)
				for (int z = 0; z < zdim; z++) {
					if (material.at(x, y, z) == Material::fluid) {
						...
						if (abs(WF_Max) < abs(WF.at(x, y, z)))
							WF_Max = WF.at(x, y, z);

                        /*
						if (abs(WF.at(x, y, z)) > 0.5) {
							cout << "x : " << x << " y: " << y << " z: " << z << endl;
							for (int k = 0; k < Q; k++) {
								int xp = x - e[k][0];
								int yp = y - e[k][1];
								...
								cout << "f " << k << " " << F.at(x, y, z, k) << " fp: " << FC.at(xp, yp, zp, r[k]) << " rho.at(xp,yp,zp): " << rho.at(xp, yp, zp) << endl;
							}
							cout << "_rho " << _rho << endl;
							cout << "U: " << U.at(x, y, z) << endl;
							cout << "V: " << V.at(x, y, z) << endl;
							cout << "W: " << W.at(x, y, z) << endl;

							cout << "UF: " << UF.at(x, y, z) << endl;
							cout << "VF: " << VF.at(x, y, z) << endl;
							cout << "WF: " << WF.at(x, y, z) << endl;

							cout << "Fx " << Force.at(x, y, z, 0) << endl;
							cout << "Fy " << Force.at(x, y, z, 1) << endl;
							cout << "Fz " << Force.at(x, y, z, 2) << endl;
						}
                        */
					}
...
#pragma omp parallel for reduction (+:temp1,temp2,total_rho)
	...
	void output_speed_field(int m) {
		ostringstream name;
		name << "speed_field" << floor_gs << "_" << m << ".data";
		ofstream out(name.str().c_str(), ofstream::binary);
		double WF_MAX = 0.0;
		for (int j = 0; j < zdim; j++)
			for (int i = 0; i < xdim; i++) {
				out.write((char*)&UF.at(i, ydim / 2, j), sizeof(double));
				out.write((char*)&WF.at(i, ydim / 2, j), sizeof(double));
				if (WF_MAX < WF.at(i, ydim / 2, j)) {
					WF_MAX = WF.at(i, ydim / 2, j);
				}
			}

		cout << "out WF_MAX: " << WF_MAX << endl;
